---
title: Accéder aux distributions des délais épidémiologiques
teaching: 20
exercises: 10
editor_options:
...
- Comment accéder aux distributions des délais épidémiologiques des maladies à partir d'une base de données préétablie pour les utiliser lors d'une analyse ?

::::::::::::::::::::::::::::::::::::::::::::::::

::::::::::::::::::::::::::::::::::::: objectives

- Obtenir les délais à partir d'une base de données de recherche documentaire avec `{epiparameter}`.
- Obtenir les paramètres de distribution et les statistiques sommaires des distributions de délais.
...
**Théorie des épidémies** Les paramètres épidémiologiques, la durée des phases d'une maladie telles que la période d'incubation, le temps de génération et l'intervalle sériel.

:::::::::::::::::::::::::::::::::

## Introduction

Les maladies infectieuses suivent un cycle infectieux qui comprend généralement les phases suivantes : période présymptomatique, période symptomatique et période de guérison, tel que leurs [histoire naturelle](../learners/reference.md#naturalhistory). Ces périodes peuvent être étudiées pour comprendre la dynamique de la transmission et informer les interventions de prévention et de contrôle des maladies.
...
Consulter le [glossaire](../learners/reference.md) pour connaître les définitions de toutes les phases que vous voyez dans la figure ci-dessus !

:::::::::::::::::::::::::

Au début d'une épidémie, les efforts visant à comprendre l'épidémie et les implications pour la controler peuvent être retardés par l'absence de moyens pouvant faciliter l'accès aux paramètres clés de la maladie d'intérêt ([Nash et al., 2023](https://mrc-ide.github.io/epireview/)). Des projets tels que `{epiparameter}` et `{epireview}` mettent fournissent une bibliothèque de paramètres épidémiologiques précédemment estimés à partir d'épidémies passées, en se basant sur des protocoles de synthèse de la littérature, qui peuvent servir dans une analyse et à paramétrer ses modèles.
...
Pour illustrer son usage dans votre pipeline d'analyse, notre objectif dans cet épisode sera d'accéder à un ensemble spécifique de paramètres épidémiologiques issus de la littérature en utilisant la librairie `{epiparameter}` de R, au lieu de les extraire des articles et les copier-coller à la main. Nous les insérerons ensuite dans un pipeline d'analyse de données basé sur `{EpiNow2}`.
...
Commençons par charger la librairue `{epiparameter}`. Nous utiliserons l'opérateur “pipe” (`%>%`) de la librairie `{magrittr}` pour connecter certaines de ses fonctions, y compris celles des packages `{tibble}` et `{dplyr}. Nous chargerons ainsi la librairie `{tidyverse}`, “mère” de ces trois packages.
...
Pour estimer la transmissibilité d'une infection, les packages les plus couramment utilisés sont `{EpiEstim}` et `{EpiNow2}`. Le package `{EpiEstim}` permet de faire une estimation en temps réel du nombre de reproductions à l'aide des données relatives aux cas enregistrés au cours du temps, reflétant ainsi l'évolution de la transmission en fonction de la date d'apparition des symptômes. Pour estimer la transmission en fonction du moment où les personnes ont été effectivement infectées (plutôt que de leurs dates d'apparition des symptômes), la librairie `{EpiNow2}` étend cette idée en la combinant avec un modèle qui tient compte des délais dans les données observées. Les deux packages requièrent certaines informations épidémiologiques en entrée. Par exemple, dans `{EpiNow2}` nous utilisons `EpiNow2::Gamma()` pour spécifier un [temps de génération](../learners/reference.md#generationtime) sous la forme d'une distribution de probabilité ajoutant sa moyenne `mean` l'écart-type (`sd`) et sa valeur maximale (`max`).

Pour spécifier un `generation_time` qui suit une distribution  *Gamma* avec une moyenne $\mu = 4$, un écart-type $\sigma = 2$ et une valeur maximale de 20, nous écrivons :
...
Il est courant que les analystes recherchent les valeurs de ces paramètres manuellement dans la littérature disponible. Une fois identifiées, ils copient et collent ces **paramètres de position** ou **paramètres de distribution** issus des publications scientifiques. Une difficulté à laquelle on est souvent confronté est que la présentation des différentes distributions statistiques n'est pas cohérente dans la littérature (par exemple, un article peut ne présenter que la moyenne, plutôt que la distribution sous-jacente complète). L'objectif de `{epiparameter}` est de faciliter l'accès à des estimations fiables des paramètres de distribution et de position pour une série de maladies infectieuses, afin qu'elles puissent être facilement utilisés dans les pipelines d'analyse des épidémies.

Dans cet épisode, nous allons *accéder* aux paramètres de position du temps de génération pour COVID-19 à partir de la bibliothèque de paramètres épidémiologiques fournie par `{epiparameter}`. Ces paramètres peuvent être utilisés pour estimer la transmissibilité de cette maladie à l'aide du package `{EpiNow2}` dans les épisodes suivants.

Commençons par examiner le nombre d'enregistrements actuellement disponibles, dans la **base de données des distributions épidémiologiques** de la librairie `{epiparameter}` à travers sa fonction `epiparameter_db()`, pour la distribution épidémiologique appelée *temps de génération* (en donnant à l'argument `epi_name` la valeur `"generation"`).
...
Dans la bibliothèque des paramètres épidémiologiques, on peut ne pas trouver un **"temps de génération"** pour la maladie qui nous intéresse. À la place, nous pouvons consulter son **"interval sériel"**. Voyons ce qu'il faut prendre en compte pour cela !

::::::::::::::::: callout

### Analyse systématique des données relatives aux agents pathogènes prioritaires

Le package [`{epireview}`](https://mrc-ide.github.io/epireview/) de R contient des paramètres sur Ebola, Marburg et Lassa issus de revues systématiques récentes. D'autres agents pathogènes prioritaires vont être inclus dans les prochaines versions. Jetez un coup d'œil à [cette vignette](https://epiverse-trace.github.io/epiparameter/articles/data_from_epireview.html) pour plus d'informations sur l'utilisation de ces paramètres avec `{epiparameter}`.

:::::::::::::::::::::::::

## Temps de génération vs intervalle sériel

Le temps de génération, conjointement avec le nombre de reproduction ($R$), peuvent fournir des indications précieuses sur le taux de croissance probable de l'épidémie et, ainsi permettre la mise en place de mesures de contrôle adéquates. Plus la valeur de $R$ est grande et/ou plus le temps de génération est court, plus le nombre de nouvelles infections attendues par jour est élevé, et donc plus l'incidence des cas de maladie augmentera rapidement.
